
# 👋 Hi there, I'm Oslan Villalobos  

💻 **DevOps Engineer | Cloud & Infrastructure Automation | Python Developer**  

I’m passionate about building scalable, automated, and resilient infrastructures in the cloud. My focus is on simplifying complexity, improving efficiency, and delivering reliable solutions.  

---

## 🚀 Skills & Tools  

- **Cloud Platforms:** ☁️ AWS | GCP  
- **Infrastructure as Code (IaC):** 🏗️ Terraform | Helm | Kubernetes Manifests  
- **Containers & Orchestration:** 🐳 Docker | Kubernetes (EKS, GKE)  
- **CI/CD Pipelines:** 🔄 GitHub Actions | Jenkins | ArgoCD  
- **Programming & Scripting:** 🐍 Python | 🐚 Bash  
- **Monitoring & Logging:** 📊 Prometheus | Grafana | ELK Stack  

---

## 📂 Featured Projects  

🔹 **[Terraform Infra](#)** → Multi-cloud infrastructure deployments using Terraform (AWS + GCP).  
🔹 **[Kubernetes Apps](#)** → Helm charts and manifests to deploy microservices on Kubernetes.  
🔹 **[CI/CD Pipelines](#)** → End-to-end automation using GitHub Actions and ArgoCD.  
🔹 **[Python Tools](#)** → Automation scripts for DevOps tasks (backups, monitoring, deployments).  
🔹 **[Bash Utils](#)** → Handy Bash scripts for server and container management.  

---

## 📊 GitHub Activity  

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Oslan17&show_icons=true&theme=tokyonight)  
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Oslan17&layout=compact&theme=tokyonight)  

---

## 🌐 Let's Connect  

- 🌎 [oslan.cloud](https://oslan.cloud)  
- 🐦 [@oslan_evp](https://x.com/oslan_evp)  
- 💼 [LinkedIn](#)  

---
